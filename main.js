...
function runSafeCommand(action) {
  if (!action || typeof action !== 'object' || action.type !== 'syscommand' || typeof action.id !== 'string') {
    console.warn('Blocked invalid command payload:', action);
    return false;
  }

  const normalized = { type: action.type, id: action.id.trim() };
    ...
    actions.forEach(action => {
      if (action && action.type === 'url' && typeof action.url === 'string') {
        try {
          ...
          console.warn('Invalid alias URL blocked:', action.url);
        }
      } else {
        runSafeCommand(action);
      }
